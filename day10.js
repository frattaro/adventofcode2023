...
const testInput = `FF7FSF7F7F7F7F7F---7
L|LJ||||||||||||F--J
FL-7LJLJ||||||LJL-77
F--JF--7||LJLJIF7FJ-
L---JF-JLJIIIIFJLJJ7
...
const startXY = map.reduce((acc, x, i) => {
  if (x.includes("S")) {
    acc[0] = i;
    acc[1] = x.findIndex(y => y === "S");
  }

  return acc;
}, [])

const loop = [startXY, [22, 90]];
// for testInput
// const loop = [startXY, [0, 3]];

while (JSON.stringify(loop[0]) !== JSON.stringify(loop[loop.length - 1])) {
  const prevY = loop[loop.length - 2][0];
  const prevX = loop[loop.length - 2][1];
  const currentY = loop[loop.length - 1][0];
  const currentX = loop[loop.length - 1][1];
  const currentPos = map[currentY][currentX];
  switch (currentPos) {
    case "|": {
      if (prevY === currentY - 1) {
        loop.push([ currentY + 1, currentX ]);
        break;
      }
    ...
    case "S": {
      throw new Error("Loop complete, shouldn't happen");
    }
  }
}
...
const vertXs = loop.map(x => x[0]);
const vertYs = loop.map(x => x[1]);

const minX = Math.min(...vertXs);
const minY = Math.min(...vertYs);
const maxX = Math.max(...vertXs);
const maxY = Math.max(...vertYs);
const stringifiedLoop = JSON.stringify(loop);
...
for (let i = 0; i < map.length; i++) {
  ...
  for (let j = 0; j < map[i].length; j++) {
    if (j < minY || j > maxY || stringifiedLoop.includes(JSON.stringify([i, j]))) {
      continue;
    }

    if (pnpoly(loop.length, vertXs, vertYs, i, j)) innerBlocks++;
  }
}
