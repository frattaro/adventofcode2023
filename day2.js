...
console.log(`Game 1: 7 red, 8 blue; 6 blue, 6 red, 2 green; 2 red, 6 green, 8 blue; 9 green, 2 red, 4 blue; 6 blue, 4 green
...
Game 100: 10 red; 11 blue, 12 red; 1 green, 7 blue, 6 red`
  .split("\n")
  .map(x => ({
    id: x.split(":")[0].match(/[\d]+/)[0],
    ...x.split(":")[1].split(";").reduce((acc, game) => {
      game.split(",").forEach(cubeGroup => {
        const number = parseInt(cubeGroup.match(/[\d]+/)[0]);
        if (cubeGroup.endsWith("red") && number > acc.r) {
          acc.r = number;
        } else if (cubeGroup.endsWith("green") && number > acc.g) {
          acc.g = number;
        } else if (cubeGroup.endsWith("blue") && number > acc.b) {
          acc.b = number;
        }
      })
  ...
  .filter(x => !(x.r > max.r || x.g > max.g || x.b > max.b))
  .reduce((acc, x) => acc + parseInt(x.id), 0)
  );
